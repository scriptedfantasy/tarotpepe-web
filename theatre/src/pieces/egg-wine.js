...
export function buildWine(ctx, obj, { switches = null } = {}) {
  ...
  // The pointer is the arbiter's (props.js SWITCHES): it owns the cursor and stops the event. An
  // empty bottle says no to it, which is the only way an empty bottle stops being a control.
  switches?.add?.({
    name: 'wine',
    object: () => obj,
    tapBox,
