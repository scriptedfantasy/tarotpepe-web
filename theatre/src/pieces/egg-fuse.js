...
export function eggFuse(ctx, { group, switches }) {
  ...
  // the arbiter in props.js raycasts the object first and falls back to the thumb's box; `over`
  // above is kept for the tools, which ask the lever directly
  switches?.add?.({ name: 'fuse', object: () => g, tapBox, onDown: () => pull() });
  return api;
}
