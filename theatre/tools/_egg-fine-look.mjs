...
// crop of each of the three places they stand in (the grate, a bookcase, the cloth's near edge).
...
const boxes = await page.evaluate(() => {
  ...
  return { n: F.lit, boxes: Array.from({ length: F.count }, (_, i) => F.flameBox(i)), where: F.where, seats: F.seats, grate: F.hitBox() };
});
console.log('lit', boxes.n, 'grate', JSON.stringify(boxes.grate));
...
const groups = { grate: boxes.boxes.slice(0, 3), shelf: boxes.boxes.slice(3, 9), table: boxes.boxes.slice(9) };
for (const [name, list] of Object.entries(groups)) {
  const b = union(list);
  const pad = 26, scale = name === 'grate' ? 6 : 3;
