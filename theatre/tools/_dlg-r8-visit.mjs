...
const LINES = ['please read my cards', 'the first one', 'the second one', 'the third one', 'thank you. good night.'];
...
await page.evaluate((lines) => {
  ...
  setInterval(() => {
    ...
    window.__probe.rows.push({
      ...
      answer: l.answer,
      text,
      beat: T.pieces.flow?.beat ?? '?', frame: T.clock?.frame ?? -1, shot: T.pieces.camera?.current ?? '?',
    });
  }, 40);
...
if (process.env.TIMELINE) {
  ...
  let prev = '';
  for (const r of rows) {
    const sig = `${r.up ? (r.dots ? 'DOTS' : r.text ? 'line' : 'bare') : 'off '} beat=${r.beat} ans=${r.answer} f=${r.frame} shot=${r.shot} “${r.text}”`;
  ...
  console.log('--- every stall in the page over 120 ms (the sampler is on the same thread) ---');
  for (let i = 1; i < rows.length; i++) if (rows[i].t - rows[i - 1].t > 120) console.log(`  ${String(rows[i - 1].t).padStart(6)} ms  stalled ${rows[i].t - rows[i - 1].t} ms  beat=${rows[i - 1].beat}`);
}
